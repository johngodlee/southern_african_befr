...
# Automate extraction of many variables in a for loop ---- 
##' Read variable for each timestep across all months for SEOSAW plots
##' List of dataframes
...
for(i in 1:length(plot_loc_list)){
  ...
  for(j in 1:length(nc_file_list)){
    ...
    sol_list <- alply(ncvar_get(nc_file_list[[j]], "var178"), 3)
    max_temp_list <- alply(ncvar_get(nc_file_list[[j]], "var201"), 3)
    min_temp_list <- alply(ncvar_get(nc_file_list[[j]], "var202"), 3)
    net_sol_list <- alply(ncvar_get(nc_file_list[[j]], "var208"), 3)
    net_sol_clear_list <- alply(ncvar_get(nc_file_list[[j]], "var210"), 3)
    precip_list <- alply(ncvar_get(nc_file_list[[j]], "var228"), 3)
    sol_down_list <- alply(ncvar_get(nc_file_list[[j]], "var178"), 3)
    
    # aggregate to daily matrices then array
    sol_array_daily <- simplify2array(
      map(split(sol_list, ceiling(seq_along(sol_list)/8)), ~reduce(.x , `+`))
    )
    max_temp_array_daily <- simplify2array(
      map(split(max_temp_list, ceiling(seq_along(max_temp_list)/8)), ~reduce(.x , `+`))
    )
    min_temp_array_daily <- simplify2array(
      map(split(min_temp_list, ceiling(seq_along(min_temp_list)/8)), ~reduce(.x , `+`))
    )
    net_sol_array_daily <- simplify2array(
      map(split(net_sol_list, ceiling(seq_along(net_sol_list)/8)), ~reduce(.x , `+`))
    )
    net_sol_clear_array_daily <- simplify2array(
      map(split(net_sol_clear_list, ceiling(seq_along(net_sol_clear_list)/8)), ~reduce(.x , `+`))
    )
    precip_array_daily <- simplify2array(
      map(split(precip_list, ceiling(seq_along(precip_list)/8)), ~reduce(.x , `+`))
    )
    sol_down_array_daily <- simplify2array(
      map(split(sol_down_list, ceiling(seq_along(sol_down_list)/8)), ~reduce(.x , `+`))
    )
    ...
    temp_df <- data.frame(time = time,
                          ...
                          lon_match = lon_match,
                          lat_match = lat_match,
                          sol = sol_array_daily[lon_match_index, lat_match_index,],
                          max_temp = max_temp_array_daily[lon_match_index, lat_match_index,],
                          min_temp = min_temp_array_daily[lon_match_index, lat_match_index,],
                          net_sol = net_sol_array_daily[lon_match_index, lat_match_index,],
                          net_clear_sol = net_sol_clear_array_daily[lon_match_index, lat_match_index,],
                          precip = precip_array_daily[lon_match_index, lat_match_index,],
                          sol_down = sol_down_array_daily[lon_match_index, lat_match_index,],
