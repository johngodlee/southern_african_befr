...
sed -i 's/caption{}/caption{Description of the biogeographical clusters to which each plot in the study was assigned. Indicator species were generated using Dufrene-Legendre indicator species analysis \\citep{Dufrene1997} implemented with \\texttt{indval()} from the \\texttt{labdsv} R package \\citep{labdsv} and represent species which define the given cluster. Dominant species were identified by choosing the species with the largest mean plot level proportional AGB contribution within each cluster. Numeric values of species richness, stems ha\\textsuperscript{-1} and AGB are medians and interquartile ranges (75th percentile - 25th percentile).}/g' "${INC}clust_summ.tex" 
